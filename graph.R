...
data <- read.csv("./results_1.csv", header = TRUE)
...
data <- data[data$max_value %in% c(1024, -1), ]
data$method <- factor(data$method)
data$size <- factor(data$size)
data$blocks <- factor(data$blocks)
data$threads <- factor(data$threads)
data$gpu <- factor(data$gpu)
data$block_threads <- with(data, interaction(blocks, threads, sep = "x"))
# data$max_value <- factor(data$max_value)
# data$max_value <- factor(data$max_value)
# my_theme <- theme(
